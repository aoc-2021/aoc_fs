...
let caveValueMap =
    let names = stringTubes |> List.map Array.toList |> List.concat |> Set |> Set.toArray
    let isUpperCase (s:string) = s.ToCharArray () |> Array.exists Char.IsUpper 
    names |> Array.mapi (fun i v -> (v,i* if isUpperCase v then -1 else 1))
    |> Map 
...
let allTubesForward: Set<Tube> =
    ...
    stringTubes |> List.map (fun s -> (s.[0] |> toCave,s.[1] |> toCave)) |> Set
    
let START = caveValueMap.TryFind "start" |> Option.get 
