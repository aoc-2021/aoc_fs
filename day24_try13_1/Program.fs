...
type Sources(inputs:Map<int,Set<int64>>) =
    ...
    override this.ToString() =
        ...
        inputs |> Map.toList |> List.map pairToString |> String.concat ","
...
type SourcedValue(vals:Set<SourcedNumber>) =
    ...
    static member Input (id:int) = 
        let input (i:int) = SourcedNumber(i|>int64,Sources([(id,[i|>int64]|>Set)] |> Map))
...
let readProgram (file:string) =
    ...
    let parseLine (line:string[]) (inputNo:int) : int*Inst=
        ...
        | [|"inp";reg|]       -> inputNo+1,INP ((toReg reg),SOURCED (SourcedValue.Input inputNo))
